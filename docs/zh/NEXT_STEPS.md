...
| RBAC 用户/Token 内存索引（认证内存化） | ✅ 已完成（2026-08-27）：`userIndex`/`tokenIndex`（engine/user_token.go）+ `ListRBACUsers`/`ListAllTokenHashes` 启动载入；`GetUserByUsername`/`GetUserByID`/`GetToken`（token 内存校验 expiry+enabled）/`PutUser`/`DeleteUserByID`/`PutTokenHash`/`DeleteTokenByHash`/`DeleteTokenByID`/`DeleteTokensByUserID`；serve 读路径 engine 优先回退 DB（`getUserByUsername`/`getToken`），写路径写穿。AIC PG 复测：3,529 → **4,111 certs/s（达 600ms 注入上限 4,167/s）**，p50 47ms→2.7ms；DB 活动仅剩批量 `INSERT INTO certificates`。一致性夹角见 `docs/RISKS.md` R11 |
| R12 — 写管线半开连接永久阻塞（MySQL+engine 写管线瘫：21GB/conn reset） | ✅ 已完成（2026-08-27）：根因双层——21GB 经 dmesg 实证为 OOM 击杀（现已有 2GiB `MaxResidentBytes` 预算兜底）；真实缺陷 = MySQL 无读超时，半开连接让 `bulkInsertChunk→Exec→readPacket` 永久阻塞并持有 `flushMu` → `Stop()→FlushAll()` 死锁。修复：mysql DSN 注入 `timeout=10s&readTimeout=30s&writeTimeout=30s`（`ensureMySQLTimeouts`）+ `ExecContext` + `BulkInsertCertRecordsCtx`/`BulkStoreDANoncesCtx` + recordbuffer `flushDBTimeout=2min` ctx 兜底。顺带 PG/MySQL 批量分块 39→**500 行/条**（`certChunkSize`，往返降 ~13×），MySQL AIC @100ms 4,325→**6,034 certs/s**。真库验证全部 exit=0 且打印报告（MySQL regular @100ms 7,575/s、AIC @100ms 6,034/s、AIC @600ms 4,114/s；PG AIC @600ms 4,054/s 无回归）。`-race` 全绿；新单测 `TestEnsureMySQLTimeouts`/`TestBulkInsertCertRecordsCtxCancelled`/`TestBulkStoreDANoncesCtxCancelled` |
| R13 — 满缓冲 DA nonce 存储打雷群涌入 flushMu（持续负载下服务器冻结） | ✅ 已完成（2026-08-27）：持续 AIC 负载下缓冲约 18s（默认 20k pending）填满，随后每个 `AddDANonce` 同步执行 `FlushAll()`——它在整个 O(积压) pass 中持有 `flushMu`——所有请求 goroutine 串行排在同一互斥锁后面，服务器冻结（40s bench 在 ~108k 成功处停滞，p99→22s，约 2k goroutine 阻塞在 `FlushAll`）。修复：满缓冲追加改为 `waitForCapacity()`——唤醒 drain 循环并在 close-and-replace 广播通道（每次 flush pass 触发）上睡眠，容量一释放全部 waiter 同时唤醒；若 5s 内无法腾出容量则 `recordbuffer.ErrBackpressure` → 归一化为 `engine.ErrBackpressure` → HTTP 503（签发失败，重放防护永不削弱）。结果：40s AIC @100ms（MySQL/engine/2500 agents）恢复到 **~163k 成功（~4.1k certs/s）** = MySQL 批量插入持续上限（独立测量 500 行 chunk ≈ 7.3k certs/s；20s 运行经缓冲吸收可突发 ~5.3k/s）；不再有 goroutine 阻塞在 `FlushAll`；背压以干净 503 呈现。新单测 `TestRecordBufferAddDANonceWaitsForCapacity`/`TestRecordBufferAddDANonceConcurrentWaits`/`TestRecordBufferAddDANonceBackpressureTimeout`；`-race` 全绿 |

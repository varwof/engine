...
**Updated (2026-08-27)** — WAL-less (non-file backend: PostgreSQL/MySQL) `StoreDANonce` now buffers the nonce through the batch write pipeline (`RecordBuffer.AddDANonce` → `BulkStoreDANonces`) instead of a synchronous single-row INSERT per request. The old path was the AIC per-request throughput wall (~3,200 certs/s ceiling, all WAL fsync per nonce). Memory remains authoritative for replay checks immediately; the backend `da_nonces` table converges on the next bulk flush. The unflushed crash window on WAL-less backends is now an **accepted tradeoff** (documented on `StoreDANonce` and `AddDANonce`) — a crash before the flush can drop unpersisted nonces, allowing a replayed DA signature until the DB converges again. WAL-enabled backends keep full crash safety. Tests: `TestStoreDANonceNoWALBatchAndReplay`, `TestDANonceNoWALBatchConvergence`, `TestRecordBufferAddDANonceNoWALBatches`.
...
serve wrapper method comments. Tested: `TestEngineLoginTokenMemoryAuthoritative`,
`TestEngineOutOfBandUserFallback`, `TestEngineUpdateUserPasswordInvalidatesTokens`,
`TestEngineUserWriteThroughAPI` (serve, race-clean) +
`TestUserTokenLoadAndAuthLookups`, `TestTokenIndexExpiryAndEnabled`,
`TestUserIndexMutations`, `TestTokenIndexLoadPutDelete` (engine).
